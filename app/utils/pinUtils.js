...
export function getInitColumns(boxes, columnNum) {
  let initColumns = new Array(columnNum).fill(0).map(() => new Array());
  boxes.map((box, i) => {
    initColumns[i%columnNum].push(box);
  });
  return Immutable.fromJS(initColumns);
}
